...
export default function VersionDeleteDialog({
  version,
}: {
  ...
  const schema = z.object({
    verify: z
      ...
      .refine((value) => value === `Delete Version ${version.version}`, {
  ...
  const onSubmit = async () => {
    const res = await apiClient.api.games.versions[':id'].$delete(
      {
        param: {
          id: version.id,
        },
    ...
    if (res.status === 204) {
      toast.success('Successfully to delete game version');
      router.refresh();
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            this game version from the server.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
